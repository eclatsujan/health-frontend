...
/**
 * Modal to submit a public review for a doctor (by profile slug).
 */
export default function ReviewModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await postJson(doctorReviewsStoreUrl(slug), {
        rating,
    ...
    >
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <p className="font-ui mt-1 text-sm text-slate-600">
          Share your experience. Reviews may be moderated.
        </p>

        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          {error ? (
            <p className="font-ui rounded-md bg-red-50 px-3 py-2 text-sm text-red-800">
              {error}
            </p>
          ) : null}
          ...
          <div>
            ...
            <p className="font-ui mt-1 text-xs text-slate-500">
              {rating} out of 5
            </p>
            {fieldErr('rating') ? (
          ...
          <div>
            <label
              htmlFor="rev-name"
              className="font-ui text-sm font-medium text-slate-700"
            >
              Your name
            </label>
            <input
