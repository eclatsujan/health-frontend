...
import LaboratoryCard from '@/components/directory/LaboratoryCard';
import type { LaboratoryCardLab } from '@/components/directory/LaboratoryCard';
import { laboratoriesIndexUrl } from '@/api/publicApi';
...
function LabsPageContent() {
  ...
  const urlForPage = useCallback(
    ...
    [search, district, rootsOnly],
  );

  const { rows, meta, loading, loadingMore, error, sentinelRef } = useInfiniteResourceList<LaboratoryCardLab>(urlForPage);
  ...
  function applyFilters(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (fd.get('roots_only') === 'on') next.set('roots_only', '1');
  ...
  return (
    <ListingPageShell
      title="Laboratories"
      subtitle="Diagnostics and partner labs. Optionally show main networks only (hide branch outlets)."
      filterLabels={[]}
    >
      <form onSubmit={applyFilters} className="mb-8 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <label htmlFor="lb-search" className="font-ui mb-1 block text-xs font-medium text-slate-600">
              Name or keyword
            </label>
            <input
              id="lb-search"
          ...
          <div>
            <label htmlFor="lb-district" className="font-ui mb-1 block text-xs font-medium text-slate-600">
              District / city
            </label>
            <input
              id="lb-district"
      ...
      {loading ? (
        <p className="font-ui py-12 text-center text-slate-500">Loading laboratories…</p>
      ) : error && rows.length === 0 ? (
        <p className="font-ui rounded-xl border border-red-200 bg-red-50 p-6 text-center text-sm text-red-800">{error}</p>
      ) : rows.length === 0 ? (
        <p className="font-ui rounded-xl border border-slate-200 bg-slate-50 py-12 text-center text-sm text-slate-600">No laboratories match your filters.</p>
      ) : (
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {rows.map((lab) => (
              <LaboratoryCard key={lab.slug} laboratory={lab} />
            ))}
          </div>
          {error && rows.length > 0 ? <p className="font-ui mt-4 text-center text-sm text-red-700">{error}</p> : null}
          <div ref={sentinelRef} className="h-px w-full" aria-hidden />
          {loadingMore ? <p className="font-ui py-8 text-center text-sm text-slate-500">Loading more laboratories…</p> : null}
